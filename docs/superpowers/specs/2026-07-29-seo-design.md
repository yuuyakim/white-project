...
現状の `address: string`（表示用の一文）は画面表示にそのまま使うため残す。構造化データ用は別フィールドとし、**「（住所確認中）」のような表示用の文字列を構造化データに流し込まない。** `PostalAddress` には確定しているフィールドだけを載せる（`addressRegion` と `addressLocality` は現時点で確定しているので常に出力し、`postalCode` と `streetAddress` は値がある会場のみ出力する）。実データは実装時に店主確認分を反映する。
